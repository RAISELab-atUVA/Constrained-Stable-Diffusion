...
### FRAMEWORK
print("\n\nPorosity conditional\n\n")
subject = "porosity_conditional"
main_dir = "/path/to/wd"
...
prompt = f"microstructures {pipe.porosity}" 
pipe.porosity = 0.3
...
if not pipe.constraint:
    print(f"Model: Conditional | Porosity: {pipe.porosity}")
    pipe.folder_name = f"run_{pipe.current_date}_conditional"

# Constraint model initialization
if pipe.constraint:
    print(f"Model: Conditional - Constraint | Porosity: {pipe.porosity}")
    pipe.folder_name = f"run_{pipe.current_date}_cond_proj_p{pipe.porosity}"
...
for i in range(num_images):
    ...
    # Constraint model
    if pipe.constraint:
        file_name = f"{i+1}_cond_proj_p{pipe.porosity}.png"

    # Conditonal model
    else:
        file_name = f"{i+1}_conditional_p{pipe.porosity}.png"
    ...
    print(f"    Image porosity: {((image) < (0.0)).float().mean()}")
