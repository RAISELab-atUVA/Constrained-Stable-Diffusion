...
class Projection:
    ...
    def bottom_k(self, tensor):
        ...
        #print(f"K: {self.k}, N° below: {below_threshold_indices.numel()}", flush=True)
        ...
        if below_threshold_indices.numel() >= self.k:
            ...
            #print(f"below, {self.k}, {tensor.numel()}")
            return tensor
        ...
        if self.baseline_1:
            ...
            #flat_tensor = torch.where(mask, torch.full_like(flat_tensor, self.threshold) + self.eps * torch.rand(flat_tensor.size()), flat_tensor).to('cuda')
        else:
            flat_tensor = torch.where(mask, torch.full_like(flat_tensor, self.threshold + self.eps), flat_tensor).to('cuda')
    ...
    def top_k(self, tensor):
        ...
        #print(f"K: {self.k}, N° above: {above_threshold_indices.numel()}", flush=True)
